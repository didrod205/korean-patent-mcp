...
- README 상단에 **데모 GIF**. 도구 3개(권리 생사 판정 → 인용 검증 → 검색)를
  20초 안에 보여준다. 733×366, 321KB.
- `scripts/make-demo.py` — GIF 생성기. 출력 형식이 바뀌면 다시 돌려 갱신한다.

내용은 전부 실제 실행 결과다. 재현한 화면이 아니라 KIPRIS·등록원부에서 받은 값이며,
데모가 실물과 다르면 문서 전체의 신뢰가 깎이기 때문이다.

프레임마다 지속시간을 따로 준다. 같은 프레임을 여러 장 반복하는 방식은
PIL이 중복을 병합해버려 의도한 멈춤이 사라진다 — 처음에 그렇게 만들어
20초짜리가 3초로 재생됐다.
...
- README에 **문제 해결** 절. 실제로 겪은 실패 4종을 로그 메시지 기준으로 정리했다.
  사용자가 보는 증상은 "서버가 안 붙는다" 하나뿐이고 원인은 클라이언트 로그에만 있다.

  | 로그 | 원인 |
  |---|---|
  | `notarget No matching version found` | 배포 직후 `@latest` 전파 지연 |
  | 로그 자체가 없음 | 클라이언트가 설정을 아직 안 읽음 |
  | `Server disconnected` 반복 | 최초 다운로드(30초) 타임아웃 |
  | `KIPRIS_SERVICE_KEY 없음` | 앱이 켜진 채 설정을 고쳐 덮어써짐 |

  `@latest` 를 빼거나 버전을 고정해 전파 창을 피하는 방법도 적었다.

문서만 바뀌었다. 동작은 1.3.3과 같으므로 이미 붙여 쓰는 클라이언트는 재시작하지 않아도 된다.

## [1.3.3]

### 고침

- **인용을 연달아 처리하면 뒤 건이 자기 명칭을 잃던 문제.** 1.3.2의 수정이
  과교정이었다. 원인은 모듈 전역 `/g` 정규식의 `lastIndex` 오염이다 —
  앞 인용에서 `exec` 로 쓴 뒤 남은 위치를 다음 인용의 `matchAll` 이 이어받아
  자기 앞의 명칭을 못 찾았다.

  정규식을 상수로 공유하지 않고 호출마다 새로 만든다.
  `lastIndex` 를 매번 리셋하는 것보다 상태를 아예 안 만드는 편이
  빠뜨릴 여지가 없다.
